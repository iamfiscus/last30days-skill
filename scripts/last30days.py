...
def _search_x(
    ...
    """Search X via twitterapi.io (primary), Bird CLI, or xAI (runs in thread).

    Args:
        x_source: 'twitterapi' (default), 'bird', or 'xai' - which backend to use
    ...
    if mock:
        raw_response = load_fixture("twitterapi_sample.json")
        x_items = twitterapi_x.parse_x_response(raw_response or {})
        return x_items, raw_response, x_error

    # Use twitterapi.io as the primary search path
    if x_source == "twitterapi":
        raw_xai = None
        try:
        ...
        if len(x_items) == 0 and not x_error:
            core = openai_reddit._extract_core_subject(topic)
            if core.lower() != topic.lower():
                try:
...
def _search_youtube_ytdlp(
    ...
    """Search YouTube via yt-dlp (runs in thread, no API key needed).
...
def _search_youtube_tubelab(
    ...
    """Search YouTube via TubeLab (runs in thread, requires API key).
...
def _run_supplemental(
    ...
    Args:
        ...
        x_source: 'twitterapi', 'bird', or 'xai'
...
def run_research(
    ...
    if sources == "web":
        ...
        # Still run YouTube in web-only mode if available
        if run_youtube:
            if progress:
        ...
        return (reddit_items, x_items, youtube_items, web_items, web_needed,
                raw_openai, raw_xai, raw_reddit_enriched,
                reddit_error, x_error, youtube_error, web_error,
                dailydev_items, raw_dailydev, dailydev_error)
    ...
    worker_count = sum([do_reddit, do_x, run_dailydev, run_youtube, bool(web_backend)])
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        ...
        if dailydev_future:
            try:
                dailydev_items, raw_dailydev, dailydev_error = dailydev_future.result(timeout=future_timeout)
                if dailydev_error and progress:
                    progress.show_error(f"DailyDev error: {dailydev_error}")
            except TimeoutError:
                dailydev_error = f"DailyDev search timed out after {future_timeout}s"
                if progress:
                    progress.show_error(dailydev_error)
            except Exception as e:
                dailydev_error = f"{type(e).__name__}: {e}"
                if progress:
...
def main():
    ...
    # Determine X source: twitterapi.io is primary if key is set
    has_twitterapi_key = bool(config.get("TWITTERAPI_IO_KEY"))
    if has_twitterapi_key:
        x_source = "twitterapi"
    else:
        # Fall back to upstream's bird/xai auto-detection
    ...
    has_tubelab = bool(config.get("TUBELAB_API_KEY"))

    # --diagnose: show source availability and exit
    if args.diagnose:
        web_source = env.get_web_search_source(config)
        # Build x_source_status if not already set (twitterapi path skips it)
        if has_twitterapi_key:
            try:
                x_source_status = env.get_x_source_status(config)
            except Exception:
                x_source_status = {"source": "twitterapi", "bird_installed": False, "bird_authenticated": False}
        diag = {
            "openai": bool(config.get("OPENAI_API_KEY")),
            "xai": bool(config.get("XAI_API_KEY")),
            "twitterapi_io": has_twitterapi_key,
            "x_source": x_source,
            "bird_installed": x_source_status.get("bird_installed", False),
    ...
    web_source = env.get_web_search_source(config)
    # Build x_source_status if not already set
    if has_twitterapi_key:
        try:
            x_source_status = env.get_x_source_status(config)
        except Exception:
    ...
    # Override available if twitterapi or Bird is ready
    if x_source == "twitterapi":
        if available == 'reddit':
            available = 'both'
        elif available == 'web':
    ...
    # Run research
    (reddit_items, x_items, youtube_items, web_items, web_needed,
     raw_openai, raw_xai, raw_reddit_enriched,
     reddit_error, x_error, youtube_error, web_error,
     dailydev_items, raw_dailydev, dailydev_error) = run_research(
        args.topic,
        sources,
        config,
        selected_models,
    ...
    if not x_source:
        if has_twitterapi_key:
            source_info["x_skip_reason"] = "TWITTERAPI_IO_KEY set but X search failed"
        else:
            try:
                if x_source_status.get("bird_installed"):
                    source_info["x_skip_reason"] = "Bird installed but not authenticated -- log into x.com in browser"
                else:
                    source_info["x_skip_reason"] = "No Bird CLI, XAI_API_KEY, or TWITTERAPI_IO_KEY"
            except Exception:
                source_info["x_skip_reason"] = "No X search API keys configured"
    if not has_ytdlp and not has_tubelab:
        source_info["youtube_skip_reason"] = "yt-dlp not installed and no TUBELAB_API_KEY -- fix: brew install yt-dlp"
    if not web_source:
    ...
    if args.store:
        ...
        for item in deduped_dailydev:
            findings.append({
                "source": "dailydev",
                "url": item.url,
                "title": item.title,
                "author": getattr(item, "source_name", "daily.dev"),
                "content": getattr(item, "snippet", item.title),
                "engagement_score": 0,
                "relevance_score": item.relevance,
            })
